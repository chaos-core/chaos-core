...
export class CoreApiClient extends ChaosApiClient {
  ...
  static async getPluginActions({pluginName}) {
    const data = await this.get(`${this.baseUrl}/plugins/${encodeURIComponent(pluginName)}/actions`);
    return data.actions;
  }
  ...
  static async runConfigAction({guildId, pluginName, actionName, args}) {
    const data = await this.post(
      `${this.baseUrl}/${guildId}/plugins/${encodeURIComponent(pluginName)}/${encodeURIComponent(actionName)}`,
      {args},
    );
