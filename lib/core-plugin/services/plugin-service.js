...
const requiredPlugins = ['core', 'web-api'];
...
class PluginService extends Service {
  ...
  async disablePlugin(guildId, pluginName) {
    ...
    if (requiredPlugins.indexOf(plugin.name) !== -1) {
      throw new PluginError(`${plugin.name} plugin can't be disabled.`);
    }
  ...
  async isPluginEnabled(guildId, pluginName) {
    ...
    if (requiredPlugins.indexOf(plugin.name) !== -1) {
      return true; // required plugins are always enabled
    }
